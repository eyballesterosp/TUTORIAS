...
public class LTutorias {
    ...
    private PrintWriter outputPer,outputTutorias,outputestu,outputTutor,outputAdm,outputhest,
            outputhtutor,outputca,outputma,outputmae,outputSoli;
        ...
        if (fichero.exists()) {
            ...
            for (int t = 0; t < as.size(); t++) {
                outputTutor.println(as.get(t).getCcTut() + "," + as.get(t).getNomTut() + ","
                        ...
                        + as.get(t).getNumTut() + "," + as.get(t).getFechaCi() + "," + as.get(t).getCodExam() + ","
    ...
    public void cargararchivotut() {
        ...
        try {
            ...
            while (tutores.readRecord()) {
                String nomtut = tutores.get(1);
                String dirtut = tutores.get(2);
                String fechaci = tutores.get(9);
                String lugarci = tutores.get(11);
                String numt = tutores.get(8);
                String cartera = tutores.get(13);
                String corrtut = tutores.get(5);
                String ubitut = tutores.get(6);
                String codexam = tutores.get(10);
                String puntaje = tutores.get(12);
                String cctut = tutores.get(0);
                String teltut = tutores.get(3);
                String edadtut = tutores.get(4);
                String usuario = tutores.get(14);
                String tipo = tutores.get(15);
                String nombreusuario = tutores.get(16);
                String contraseña = tutores.get(17);
    ...
    public void estudiante(String nomEst, String dirEst, String corrEst, String ubiEst, long ccEst, long telEst, long edadEst, long cartera, String persona, String tipo, String nombrepersona, String contraseña) {
        est.add(new Estudiante(nomEst, dirEst, corrEst, ubiEst, ccEst, telEst, edadEst, cartera, persona, tipo, nombrepersona, contraseña));
    ...
    public void cargararchivoest() {
        ...
        try {
            ...
            while (estudiantes.readRecord()) {
                String nomEst = estudiantes.get(1);
                String dirEst = estudiantes.get(2);
                String corrEst = estudiantes.get(5);
                String ubiEst = estudiantes.get(6);

                String ccEst = estudiantes.get(0);
                String telEst = estudiantes.get(3);
                String edadEst = estudiantes.get(4);
                String usuario = estudiantes.get(7);
                String tipo = estudiantes.get(8);
                String nombreUsuario = estudiantes.get(9);
                String contraseña = estudiantes.get(10);
                ...
                est.add(new Estudiante(nomEst, dirEst, corrEst, ubiEst, ccEsti, telEsti, edadEsti, carteraa, usuario, tipo, nombreUsuario, contraseña));
            }
...
//NOMBRE,DIRECCION,CORREO,CEDULA,TELEFONO,CODIGO,USUARIO, TIPO,NOMBRE USUARIO, CONTRASEÑA
                ...
                String corradm = admin.get(2);
                String ccadm = admin.get(3);
                String teladm = admin.get(4);
                String codadm = admin.get(5);
                String usuario = admin.get(6);
                String tipo = admin.get(7);
                String nombreusuario = admin.get(8);
                String contraseña = admin.get(9);
...
//CEDULA,HORA,LUNES,MARTES,MIERCOLES,JUEVES,VIERNES
                String cctutor = hortu.get(0);
                String hora = hortu.get(1);
                String lunes = hortu.get(2);
                String martes = hortu.get(3);
                String miercoles = hortu.get(4);
                String jueves = hortu.get(5);
                String viernes = hortu.get(6);
...
//CEDULA,LUNES,MARTES,MIERCOLES,JUEVES,VIERNES
                String ccest = horest.get(0);
                String hora = horest.get(1);
                String lunes = horest.get(2);
                String martes = horest.get(3);
                String miercoles = horest.get(4);
                String jueves = horest.get(5);
                String viernes = horest.get(6);
...
//CEDULA,NOMBRE,CODIGO EXAMEN,FECHA,PUNTAJE,LUGAR
                String cctut = cal.get(0);
                String nombreaspirante = cal.get(1);
                String codigo = cal.get(2);
                String fechaci = cal.get(3);
                String puntaje = cal.get(4);
                String lugarci = cal.get(5);
    ...
    public void crearcvsma() throws IOException {
        ...
        if (f5.exists()) {
            ...
            for (int t = 0; t < ma.size(); t++) {
                outputma.println(ma.get(t).getId()+ "," + ma.get(t).getMateria());
...
//CEDULA,NOMBRE,CODIGO EXAMEN,FECHA,PUNTAJE,LUGAR
                String cctut = mat.get(0);
                String materia = mat.get(1);
                long lcctut = Long.parseLong(cctut);
    ...
    public void crearcvsmae() throws IOException {
        ...
        if (f5.exists()) {
            ...
            for (int t = 0; t < mae.size(); t++) {
                outputmae.println(mae.get(t).getId() + "," + mae.get(t).getMateria());
...
//CEDULA,NOMBRE,CODIGO EXAMEN,FECHA,PUNTAJE,LUGAR
    ...
    public void creartu(String nomtutor, String nomreestu, String lugartu, String fechatut, String horatut, String tematut, String estado, long tel, long costo, String calt, String come) {
        tutorias.add(new Tutorias(nomtutor, nomreestu, lugartu, fechatut, horatut, tematut, estado, calt, come, tel, costo));
    ...
    public void crearcvstu() throws IOException {
        ...
        if (f5.exists()) {
            ...
            for (int t = 0; t < tutorias.size(); t++) {
                outputTutorias.println(tutorias.get(t).getNameTutor() + "," + tutorias.get(t).getNameEst() + "," + tutorias.get(t).getLugarTu() + ","
                        + tutorias.get(t).getFechaTut() + "," + tutorias.get(t).getHoraTut() + "," + tutorias.get(t).getTemaTut()
                        + "," + tutorias.get(t).getEstado() + "," + tutorias.get(t).getTel() + "," + tutorias.get(t).getCosto() + "," + tutorias.get(t).getCalTut()
...
//NOMBRE TUTOR,NOMBRE ESTUDIANTE, LUGAR, FECHA,HORA,TEMA,ESTADO,TELEFONO ESTUDIANTE,COSTO
                String nom = tutori.get(0);
                String nombrees = tutori.get(1);
                String lugar = tutori.get(2);
                String fecha = tutori.get(3);
                String hora = tutori.get(4);
                String tema = tutori.get(5);
                String estado = tutori.get(6);
                String tel = tutori.get(7);
                String costo = tutori.get(8);
                String calt = tutori.get(9);
                String come = tutori.get(10);

                long ltel = Long.parseLong(tel);
                long lcosto = Long.parseLong(costo);

                tutorias.add(new Tutorias(come, nombrees, lugar, fecha, hora, tema, estado, calt, come, ltel, lcosto));
            }
    ...
    public void crearsoli(String nombreDe, String NombrePara, long codigoSolicitud, String solicitud, String respuesta) {
        soli.add(new Solicitudes(nombreDe, NombrePara, solicitud, respuesta, codigoSolicitud));
    ...
    public void crearcvssol() throws IOException {
        ...
        if (f4.exists()) {
            ...
            for (int t = 0; t < soli.size(); t++) {
                outputSoli.println(soli.get(t).getNombreRemitente()+ "," + soli.get(t).getNombreDestinatario()+ "," + soli.get(t).getIdSolicitud()+ "," + soli.get(t).getSolicitud() + "," + soli.get(t).getRespuesta());
    ...
    public void cargararchivosol() {
        ...
        try {
            ...
            while (sol.readRecord()) {
                ...
                soli.add(new Solicitudes(nombreEst, nombreEst, solicitud, respuesta, codsol));
            }
